  ...
  return (
    <Panel>
      ...
      <ul className="max-h-[560px] overflow-y-auto">
        {rows.map((row) => (
          <li key={row.id}>
            <Link
              href={href(row.id)}
              className={cn(
                "flex items-center justify-between gap-2 border-b px-card py-3 text-sm hover:bg-secondary",
                row.id === selectedId && "bg-secondary font-medium",
              )}
            >
              <span className="min-w-0">
                <span className="block truncate">{row.name}</span>
                <span className="block text-xs text-muted-foreground tabular-nums">{row.phone}</span>
              </span>
              {row.rates > 0 ? (
                <span className="shrink-0 rounded-full bg-warning-soft px-2 py-0.5 text-2xs text-warning">
                  {row.rates} special
                </span>
              ) : null}
            </Link>
        ...
        {rows.length === 0 ? <li className="px-card py-8 text-center text-muted-foreground">No customer matches that</li> : null}
      </ul>
